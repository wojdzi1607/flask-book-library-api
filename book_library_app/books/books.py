...
from book_library_app.utils import validate_json_content_type, token_required, get_schema_args, apply_order, apply_filter, get_pagination
...
def get_book(book_id: int):
    book = Book.query.get_or_404(book_id, description=f'Book with id {book_id} is not exists')
    return jsonify({
        'success': True,
...
def update_book(user_id: int, args: dict, book_id: int):
    ...
    if author_id is not None:
        Author.query.get_or_404(author_id, description=f'Author with id {author_id} is not exists')
